...
const CopyToClipboardModal = ({ message, onClose }) => {
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-50">
      <div className="bg-white p-4 rounded-lg shadow-lg w-80">
        ...
        <button
          onClick={onClose}
          className="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-200 w-full"
        >
          Close
        </button>
      </div>
...
const AccordionList = ({ title, resources_topicName, concepts_subTopic }) => {
  ...
  const handleCheckItem = (concept) => {
    setCheckedItems((prev) => ({
      ...prev,
      [concept.topic]: !prev[concept.topic],  // Toggle checkbox state
    }));
  };
                ...
                <div key={index} className="py-3">
                  ...
                  {expandedItems.includes(concept.topic) && (
                    <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                      ...
                      <p className="text-gray-700 mb-2">
                        <strong>Code:</strong>
                        <div className="relative bg-gray-800 text-white rounded-lg border border-gray-600">
                          <pre className="p-4">
                            <code className="text-sm font-mono whitespace-pre-wrap">{concept.code}</code>
                          </pre>
                          {/* Copy Button */}
                          <button
                            onClick={() => copyToClipboard(concept.code)}
                            className="absolute top-2 right-2 p-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-200"
                          >
